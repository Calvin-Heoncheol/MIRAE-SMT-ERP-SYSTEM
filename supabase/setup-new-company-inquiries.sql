...
comment on column public.new_company_inquiries.close_reason is '종료 사유 (상태=closed 일 때)';
comment on column public.new_company_inquiries.created_by is '등록자 auth.users.id';
comment on column public.new_company_inquiries.created_by_name is '등록자 표시명 스냅샷';
...
create index if not exists new_company_inquiries_created_by_idx
  on public.new_company_inquiries (created_by);
...
begin
  ...
  new.close_reason := coalesce(trim(new.close_reason), '');
