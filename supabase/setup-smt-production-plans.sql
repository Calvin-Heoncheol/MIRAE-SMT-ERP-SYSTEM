...
comment on column public.smt_production_plans.created_by is '등록자 auth.users.id';
comment on column public.smt_production_plans.created_by_name is '등록자 표시명 스냅샷';
...
create index if not exists smt_production_plans_created_by_idx
  on public.smt_production_plans (created_by);
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'smt_production_plans_plan_status_check'
  ) then
    alter table public.smt_production_plans
