...
create table if not exists public.smt_production_plans (
  ...
  unique (order_id, planned_date, line_no)
);

comment on table public.smt_production_plans is 'SMT 생산계획 — 주문서·일자·라인별 계획 수량';
...
comment on column public.smt_production_plans.planned_quantity is '해당 일·라인 계획 수량';
