...
begin
  ...
  if coalesce(trim(p_assembly_group_id), '') = '' then
  ...
  perform pg_advisory_xact_lock(hashtext('delivery:' || p_assembly_group_id));
...
begin
  ...
  perform pg_advisory_xact_lock(hashtext('post:' || p_assembly_group_id));
