...
-- - 유니크는 (base_code, name, version) — migrate-items-unique-code-name-version.sql 참고
...
-- 유니크: 품목코드 + 품명 + 버전 (같은 코드라도 품명·버전 다르면 허용)
drop index if exists items_base_code_version_uidx;
drop index if exists items_base_code_name_version_uidx;
