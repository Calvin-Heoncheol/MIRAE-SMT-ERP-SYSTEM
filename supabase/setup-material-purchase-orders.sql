...
comment on column public.material_purchase_orders.created_by is '등록자 auth.users.id';
comment on column public.material_purchase_orders.created_by_name is '등록자 표시명 스냅샷';
...
create index if not exists material_purchase_orders_created_by_idx
  on public.material_purchase_orders (created_by);
