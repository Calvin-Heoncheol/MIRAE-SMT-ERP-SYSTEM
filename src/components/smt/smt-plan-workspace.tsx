...
export function SmtPlanWorkspace({ initialResult, initialWeekStart }: SmtPlanWorkspaceProps) {
  ...
  const unplannedCandidates = useMemo(() => {
    return (data?.planCandidates ?? []).filter((candidate) => candidate.unplannedRemaining > 0)
  }, [data?.planCandidates])
  ...
  function openCreateModal(order: SmtPlanOrderCandidate, plannedDate: string, lineNo: number) {
    ...
    const plannedQuantity = suggestPlanQuantityFromMaterial(
      maxQuantity,
      order.materialReadyUnits ?? 0,
    )
    setModal({
      open: true,
      mode: 'create',
  ...
  function openCreateFromCell(plannedDate: string, lineNo: number) {
    setStatusMessage('')
    setModal({
      open: true,
      mode: 'create',
  ...
  function handlePickCandidate(order: SmtPlanOrderCandidate) {
    ...
    openCreateModal(order, modal.initialValues.plannedDate, modal.initialValues.lineNo)
  }
  ...
  function openEditModal(plan: SmtPlanBlock) {
    ...
    setModal({
      ...
      initialValues: {
        ...
        plannedEndDate: plan.plannedEndDate || plan.plannedDate,
        planStatus: plan.planStatus || 'confirmed',
        lineNo: plan.lineNo,
  ...
  function openMoveModal(plan: SmtPlanBlock, plannedDate: string, lineNo: number) {
    ...
    setModal({
      ...
      initialValues: {
        ...
        orderLineId: plan.orderLineId,
        plannedDate,
        plannedEndDate: plan.plannedEndDate || plannedDate,
        planStatus: plan.planStatus || 'confirmed',
        lineNo,
        pcbSide: plan.pcbSide,
        plannedQuantity: plan.plannedQuantity,
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden p-3">
          ...
          {loading ? (
            ...
            <SmtPlanCalendar
              ...
              activeDropCell={null}
              onDrop={handleDrop}
              onCellClick={({ plannedDate, lineNo }) => openCreateFromCell(plannedDate, lineNo)}
