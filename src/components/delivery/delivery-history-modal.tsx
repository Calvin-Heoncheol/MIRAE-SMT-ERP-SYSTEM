...
import {
  ...
  fetchDeliveryHistoryByShipmentId,
...
export function DeliveryHistoryModal({
  ...
  async function handlePrintStatement() {
    const shipmentId = row!.shipmentId || row!.id
    const history = await fetchDeliveryHistoryByShipmentId(shipmentId)
    if (!history.ok) {
      setSaveError(history.detail)
      return
    }

    const lines = history.rows.length ? history.rows : [row!]
    const shippedLines = []
    for (const line of lines) {
      ...
      if (line.id === row!.id) {
        ...
        const priceResult = await fetchOrderLineUnitPrice(line.orderNumber, line.productCode)
        unitPrice = priceResult.ok ? priceResult.unitPrice : 0
      }
      shippedLines.push({
        orderNumber: line.orderNumber,
        productCode: line.productCode,
        productName: line.productName,
        qty: line.id === row!.id && qtyNumber > 0 ? qtyNumber : line.quantity,
        unitPrice,
      })
    }
