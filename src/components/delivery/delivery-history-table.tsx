...
type DeliveryHistoryTableProps = {
  groups: DeliveryHistoryShipmentGroup[]
  emptyMessage: string
  onRowClick?: (group: DeliveryHistoryShipmentGroup) => void
}
...
export function DeliveryHistoryTable({ groups, emptyMessage, onRowClick }: DeliveryHistoryTableProps) {
  if (!groups.length) {
    return (
          ...
          <tbody>
            {groups.map((group) => (
              ...
              >
                ...
                <td className="px-3 py-2.5 text-sm text-slate-800">{cell(group.productName)}</td>
