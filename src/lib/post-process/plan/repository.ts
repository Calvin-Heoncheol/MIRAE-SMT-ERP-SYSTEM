...
import {
  ...
  validatePostAfterSmt,
} from '@/lib/production-plan/schedule'
...
const PLAN_SELECT =
  'id, order_id, assembly_group_id, planned_date, planned_end_date, plan_status, team, planned_quantity, note, created_by, created_by_name, created_at'
const PLAN_SELECT_LEGACY =
  'id, order_id, assembly_group_id, planned_date, team, planned_quantity, note, created_at'

function isMissingPlanScheduleColumns(detail: string) {
  const message = detail.toLowerCase()
  return (
    (message.includes('planned_end_date') || message.includes('plan_status')) &&
...
function mapPlan(row: {
  ...
  const plannedDate = String(row.planned_date || '').slice(0, 10)
  return {
    id: row.id,
    orderId: row.order_id,
    assemblyGroupId: String(row.assembly_group_id || '').trim(),
    plannedDate,
    plannedEndDate: resolvePlannedEndDate(plannedDate, row.planned_end_date),
    planStatus: normalizeProductionPlanStatus(row.plan_status),
...
export async function upsertPostProcessProductionPlan(
  ...
  const plannedEndDate = resolvePlannedEndDate(plannedDate, input.plannedEndDate)
  const planStatus = normalizeProductionPlanStatus(input.planStatus)
  ...
  if (plannedEndDate < plannedDate) {
    return { ok: false, reason: 'validation', detail: '후공정 종료일은 시작일 이후여야 합니다.' }
  }
  ...
  try {
    const smtPlansResult = await fetchAllSmtProductionPlans()
    if (smtPlansResult.ok) {
      const smtEnds = smtPlansResult.plans
        .filter((plan) => plan.orderId === orderId)
        .map((plan) => plan.plannedEndDate || plan.plannedDate)
      const timing = validatePostAfterSmt({ postStart: plannedDate, smtEndDates: smtEnds })
      if (!timing.ok) {
        return { ok: false, reason: 'validation', detail: timing.detail }
      }
    }
    ...
    if (error && (isMissingCreatedByColumn(error.message) || isMissingPlanScheduleColumns(error.message))) {
      insertPayload = stripCreatedByFields({
        ...basePayload,
        created_by: snap.createdBy,
        created_by_name: snap.createdByName,
      })
