...
async function loadItemMeta(
  ...
  | {
      ok: true
      byId: Record<string, { category: ItemCategory; baseCode: string }>
    }
...
> {
  ...
  const { data, error } = await supabase
    .from('items')
    .select('id, item_category, base_code')
    .in('id', ids)
  if (error) return { ok: false, detail: error.message }

  const byId: Record<string, { category: ItemCategory; baseCode: string }> = {}
...
export async function syncFinishedParentUnitPriceFromBom(
  ...
  try {
    const supabase = createSupabaseClient()
    const categoriesResult = await loadItemMeta(supabase, [parentId])
    if (!categoriesResult.ok) {
      return { ok: false, reason: 'query', detail: categoriesResult.detail }
    }

    if (categoriesResult.byId[parentId]?.category !== 4) {
      return { ok: true, unitPrice: 0, lineCount: 0 }
    }
...
export async function saveBomForParent(
  ...
  try {
    ...
    const metaResult = await loadItemMeta(supabase, [parentId, ...childIds])
    if (!metaResult.ok) {
      return { ok: false, reason: 'query', detail: metaResult.detail }
    }

    const parentCategory = metaResult.byId[parentId]?.category
    ...
    for (const line of lines) {
      ...
      const childMeta = metaResult.byId[childId]
      if (!childMeta) {
        return { ok: false, reason: 'validation', detail: `구성 품목 ${childId} 을(를) 찾을 수 없습니다.` }
      }
      const childCategory = childMeta.category
      ...
      if (parentCategory === 3 && isRawMaterialItemCategory(childCategory)) {
        const rawCode = (childMeta.baseCode || childId).toLowerCase()
        if (seenRawBaseCodes.has(rawCode)) {
          return {
            ok: false,
            reason: 'validation',
            detail: `원자재 품목코드 ${childMeta.baseCode || childId} 이(가) BOM에 중복되었습니다.`,
          }
        }
