...
/** Excel/CSV 붙여넣기 시 남는 따옴표 제거 ("MR-001" → MR-001) */
...
function normalizePasteCell(value: string) {
  return stripExcelQuotes(value)
}

function isHeaderLine(line: string) {
  const first = normalizePasteCell(splitPasteColumns(line)[0] || '')
...
export function parseBomBulkPaste(text: string): BomPasteRow[] {
  ...
  for (let i = startIndex; i < lines.length; i += 1) {
    const cols = splitPasteColumns(lines[i]).map(normalizePasteCell)
  ...
  if (!lines.length) {
    return {
      ...
      detail: unresolved.length
        ? `일치하는 품목이 없습니다. (예: ${unresolved.slice(0, 3).map((row) => row.token).join(', ')})`
        : '붙여넣을 내용이 없습니다.',
      unresolved,
    }
  }
