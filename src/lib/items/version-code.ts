...
export function normalizeVersionLabel(versionInput: string | number): string {
  // 앞뒤 공백만 제거 — 가운데 띄어쓰기는 유지 (예: "REV 2")
...
function isBaseVersionTaken(
  existingItems: Pick<Item, 'id' | 'baseCode' | 'version'>[],
  baseCode: string,
  version: string,
) {
  const base = baseCode.trim().toLowerCase()
  const ver = normalizeVersionLabel(version).toLowerCase()
  return existingItems.some((row) => {
    const resolved = resolveItemBaseAndVersion(row)
    return (
      resolved.base.trim().toLowerCase() === base &&
      normalizeVersionLabel(resolved.version || '').toLowerCase() === ver
    )
  })
}

/** base_code 기준으로 다음 버전 행 제안. 내부 PK는 저장 시 새로 발급 */
export function suggestNextVersionForItem(
  ...
  if (isBaseVersionTaken(existingItems, base, parsed.version)) {
    for (let n = 1; n <= 999; n += 1) {
      const candidate = `V${n}`
      if (!isBaseVersionTaken(existingItems, base, candidate)) {
        return { newId: '', baseCode: base, version: candidate }
      }
    }
    return null
  }
...
export function resolveManualVersionForItem(
  ...
  if (isBaseVersionTaken(existingItems, base, version)) {
    return {
      ok: false,
...
export function itemToVersionUpPayload(
  ...
  const parsed = parseItemVersionCode(newId)
  const baseCode = (options?.baseCode || item.baseCode || parsed.base || item.id).trim()
