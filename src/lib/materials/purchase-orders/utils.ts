...
export function resolveMaterialPurchaseInboundStatus(
  order: Pick<MaterialPurchaseOrderListGroup, 'items'>,
): MaterialPurchaseInboundStatus {
  const items = order.items
  if (!items.length) return 'none'

  let hasAnyInbound = false
  let allDone = true
  for (const item of items) {
    const ordered = Math.max(0, Number(item.quantity) || 0)
    const received = Math.max(0, Number(item.inboundQuantity) || 0)
    if (received > 0) hasAnyInbound = true
    if (ordered <= 0) {
      if (received > 0) allDone = false
      continue
    }
    if (received < ordered) allDone = false
  }

  if (!hasAnyInbound) return 'none'
  if (allDone) return 'done'
  return 'partial'
}

export function getMaterialPurchaseInboundStatusLabel(status: MaterialPurchaseInboundStatus) {
  if (status === 'done') return '입고완료'
  if (status === 'partial') return '부분입고'
  return '미입고'
}

export function getMaterialPurchaseInboundStatusClassName(status: MaterialPurchaseInboundStatus) {
...
export function resolveMaterialPurchaseOrderLineMaterial(
  ...
  if (code) {
    ...
    const byId = resolveMaterialById(materials, code)
    if (byId) return byId
  }

  return (
    resolveMaterialFromInput(materials, supplier, item.materialName, materialCode) ||
    resolveMaterialFromInput(materials, null, item.materialName, materialCode)
  )
}
