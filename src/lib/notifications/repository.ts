...
const SOURCES: ActivitySource[] = [
  ...
  },
  {
    ...
    href: '/delivery/input',
    select:
      'id, shipment_id, quantity, record_date, note, created_at, created_by, created_by_name',
    build: (row) => {
      const id = String(row.id || '').trim()
      if (!id) return null
  ...
  const batches = await Promise.all(
    SOURCES.map(async (source) => {
      ...
      for (const row of rows) {
        ...
        // 출하는 같은 shipment_id 묶음을 한 알림으로 합침
        const shipmentId =
          source.kind === 'delivery'
            ? String(row.shipment_id || row.id || '').trim()
            : ''
        const rowId =
          source.kind === 'delivery' && shipmentId
            ? `ship:${shipmentId}`
            : String(row.id || createdAt)

        const existing = items.find((item) => item.key === `${source.kind}:${rowId}`)
        if (existing && source.kind === 'delivery') {
          const qty = Number(row.quantity) || 0
          const prevQtyMatch = existing.detail.match(/([\d,]+)EA/)
          const prevQty = prevQtyMatch
            ? Number(String(prevQtyMatch[1]).replace(/,/g, '')) || 0
            : 0
          const nextQty = prevQty + qty
          const date = String(row.record_date || '').trim()
          existing.detail = `${shipmentId} · ${nextQty.toLocaleString('ko-KR')}EA${
            date ? ` · ${date}` : ''
          }`
