...
export async function fetchProductionInputPageData(
  ...
  if (config.productionModule === 'smt') {
    ...
    const [countsResult, smtOrdersResult, assemblyResult, deliveryCountsResult, quotesResult] =
      await Promise.all([
  ...
  if (config.productionModule === 'post_process') {
    ...
    const [assemblyFetchResult, countsResult, deliveryCountsResult, quotesResult] = await Promise.all([
      fetchAssemblyGroups(productById),
  ...
  const [ordersResult, productsResult, quotesResult] = await Promise.all([
    fetchOrders(),
    fetchProducts(false),
    fetchQuotes(),
  ])
