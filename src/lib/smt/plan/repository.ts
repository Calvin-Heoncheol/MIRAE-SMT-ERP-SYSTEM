...
const SMT_PLAN_SELECT =
  'id, order_id, order_line_id, planned_date, planned_end_date, plan_status, line_no, pcb_side, planned_quantity, note, created_by, created_by_name, created_at'
const SMT_PLAN_SELECT_LEGACY =
...
export async function fetchSmtPlanPageData(weekStart: string): Promise<FetchSmtPlanPageResult> {
  ...
  const planCandidates: SmtPlanOrderCandidate[] = buildSmtPlanOrderCandidates(
    ...
    return {
      ...
      materialReadyUnits: materialInbound.readyUnits,
      materialScheduledUnits: materialInbound.scheduledUnits,
    }
  })
...
export async function upsertSmtProductionPlan(
  ...
  if (plannedEndDate < plannedDate) {
    return { ok: false, reason: 'validation', detail: 'SMT 종료일은 시작일 이후여야 합니다.' }
  }
